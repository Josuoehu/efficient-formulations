...
def translate_task(
    ...
    #Cristina
    #print("strips_to_sas:",strips_to_sas)
    #print("init: ",init)
    static_initFacts = list(filter(lambda x: x not in strips_to_sas, init))
    printStaticFacts(static_initFacts, sys.stdout)
    with open(options.sas_file, "w") as output_file:
    	printStaticFacts(static_initFacts, output_file)
    #end-Cristina
    if options.dump_task:
        ...
        print("nonconstant_init:",nonconstant_init);
...
def main():
    ...
    with timers.timing("Writing output"):
        #with open(options.sas_file, "w") as output_file:
        with open(options.sas_file, "a") as output_file:
...
#Cristina
def printStaticFacts(listStatic, stream):
    print("begin_static", file=stream)
    for elem in listStatic:
        if elem.predicate != "=":
            print(elem, file=stream)
    print("end_static", file=stream)
#end-Cristina
